...
// NOTE: Unlike wallet RPC (which use SSF values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    if (block.vtx.empty() || !(block.vtx[0]->IsCoinBase() || (block.vtx.size() > 1 && block.vtx[1]->IsCoinStake()))) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block does not start with a coinbase/coinstake");
    }
